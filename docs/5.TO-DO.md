...
> 작성일: 2026-03-04  
> **구현 완료 항목**: RFP Chunking(최대 60,000자), Industry Stats, Slide Quality Scorer, Cross-Phase Context, Phase Checkpoint, Negative Prompts, Company Profiler, setup-company, 폰트 폴백, PROMPT_VERSION, **수동 모드(Manual Mode)** — `generate --manual` / `continue` / `status`, 9단계 파일 기반(N_step_{Phase명}_request/response.txt), ManualOrchestrator·state.json, **수동 모드 브라우저 자동화** — `manual-run --site gemini` / `--site chatgpt` (Playwright, 각 단계마다 새 브라우저, RFP 지정 시 항상 새 run), `--skip-login-wait`, `--headless`, **체크포인트 재개** — `generate --resume-checkpoint`, run_metadata.json, **PPTX 생성** — 템플릿 옵션 제거, _no_template_mode·플레이스홀더 정리로 디자인 깨짐 방지, **DOCX 파서** — 헤더/바닥글·문서구조·메타데이터 추출, **analyze** — 핵심 요구사항·평가 기준·산출물 상세 목록 출력.
