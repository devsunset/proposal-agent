...
> 작성일: 2026-03-04  
> **구현 완료 항목**: RFP Chunking, Industry Stats, Slide Quality Scorer, Cross-Phase Context, Phase Checkpoint, Negative Prompts, Company Profiler, setup-company, 폰트 폴백, PROMPT_VERSION, **수동 모드(Manual Mode)** — `generate --manual` / `continue` / `status`, 9단계 파일 기반(N_step_{Phase명}_request/response.txt), ManualOrchestrator·state.json, **수동 모드 브라우저 자동화** — `manual-step --site gemini` / `--site chatgpt` (Playwright 기반, request 파일 파싱 후 Gemini/ChatGPT 웹에 전송·응답 저장), **체크포인트 재개** — `generate --resume-checkpoint _checkpoints/run_YYYYMMDD_HHMMSS`, run_metadata.json, ProposalOrchestrator.resume_from_checkpoint, ContentGenerator.execute_from_phase, **PPTX 생성 버그 수정**(2026-03-05) — `_find_guide_template()`이 레이아웃 수 기준으로 `guide_template_sub.pptx` 자동 선택, `load_template()` 모든 경로에서 13.33"×7.5" 강제, `slide_layouts.json` 생성(guide_template_sub 레이아웃 매핑), 모든 `slide_layouts[6]` 하드코딩 → `min(6, len-1)` 안전 패턴, `add_title_slide()` 텍스트박스 폴백, `channel_strategy` 단일 객체 처리 수정, Pydantic v2 `.model_dump()` 적용.
...
# ✅ 구현됨: main.py에 --resume-checkpoint 옵션 추가 (체크포인트 재개)
# python main.py generate --resume-checkpoint run_YYYYMMDD_HHMMSS
# - output/_checkpoints/run_*/ 에서 phase_*.json 스캔, run_metadata.json 로드
# - 부족한 Phase만 API로 생성 후 PPTX까지 진행. run_metadata 없으면 RFP 경로 필수.
```
