...
| **Python**    | 3.10 이상                                                                                         |
| **pip**       | 최신 버전 권장                                                                                    |
| **LLM**       | Claude / Gemini / Groq 중 하나의 API 키, 또는 **Ollama**(로컬, API 키 불필요) — `.env`의 `LLM_PROVIDER`에 맞춤 |
...
| OS            | 방법                                                                                                                                 |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **Windows**   | [ollama.com/download](https://ollama.com/download) 에서 **OllamaSetup.exe** 다운로드 후 실행. 터미널에서 `ollama --version` 으로 확인. |
| **macOS**     | [ollama.com/download](https://ollama.com/download) 에서 macOS용 설치 → **Applications** 로 설치 후 앱 실행. 메뉴바에 Ollama 아이콘이 보이면 동작 중. |
...
| **model not found**            | `OLLAMA_MODEL` 이 `ollama list` 에 나오는 이름과 다름. `ollama pull <모델명>` 후 동일한 이름으로 `.env` 설정. |
| **응답이 느림**                 | 모델 크기·PC 사양(특히 GPU) 영향. 더 작은 모델(예: llama3.2)로 시험하거나 GPU 드라이버 확인.     |
