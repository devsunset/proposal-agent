...
- RFP Chunking: 섹션 우선순위 분류 → 평가기준·요구사항 우선 포함 (최대 60,000자)
...
각 명령의 **필수 인자**와 **생략 시 기본값**은 `python main.py help` 또는 [2.INSTALL_AND_USAGE.md](2.INSTALL_AND_USAGE.md) § 명령별 필수·선택 요약을 참고하세요.
...
# 수동 모드 (LLM API 없이): generate --manual → manual-run(1~9 자동) 또는 수동 응답 저장 후 continue 반복
python main.py generate input/rfp.pdf --manual -n "프로젝트명" -c "발주처"
python main.py manual-run --site gemini   # 또는 --site chatgpt. 각 단계마다 새 브라우저 실행, Step 1 로그인 후 같은 터미널 Enter → 2~9 자동 진행
python main.py manual-run --site gemini input/rfp.pdf --skip-login-wait --headless   # 이미 로그인·테스트 시 Enter 대기 생략, 헤드리스
...
- HIGH 섹션 전문 포함, MEDIUM/LOW 구간별 제한으로 최대 60,000자 구성 (.env RFP_CHUNK_MAX_CHARS)
