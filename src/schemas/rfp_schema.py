...
def _parse_weight(value: Any) -> Optional[float]:
    """LLM이 반환한 weight 값을 float으로 정규화 (문자열 배점 설명 → 숫자)."""
    if value is None:
        return None
    ...
    if num_match:
        try:
            ...
            pass
    # 한글 배점 설명 → 추정 배점
    if "최고" in s or "높음" in s:
        return 30.0
    if "고배점" in s or "높은" in s:
        return 25.0
    if "중배점" in s or "중간" in s:
        return 15.0
    if "저" in s or "낮" in s:
        return 10.0
    return None
...
class EvaluationCriterion(BaseModel):
    ...
    Attributes:
        category: 분야 (기술, 가격 등)
        item: 평가 항목 내용
        weight: 배점 (비율, 숫자 또는 '고배점 추정' 등 문자열 시 추정치로 변환)
        description: 부가 설명
    """
    category: str
    item: str
    ...
    @classmethod
    def normalize_weight(cls, v: Any) -> Optional[float]:
        return _parse_weight(v)
